...
chatStyle.className = 'wizzchat-style';
...
const updateChatBody = () => {
    ...
    chatBody.innerHTML = chatData.map(data => `<p>${data.answer}</p>`).join('');
}
...
const fetchChatParams=async()=>{
    ...
    welcomeMessage = chatParams['welcomeMessage'];
    headerColor = chatParams['chatColor'];
    chatIcon = chatParams['chatIcon'];
    chatCoordinates = chatParams['chatCoordinates']?.split(",");
    console.log("passed");
    applyChatParams();
    chatData = [{ "answer": welcomeMessage || "" }];
    updateChatBody();
}
...
//       newStyle.className = 'wizzchat-style';
...
chatbotWrapper.innerHTML = `
  <div class="chatbot-container collapsed">
    <div class="chatbot-header">
      <img src="https://res.cloudinary.com/dswjg0rjx/image/upload/v1721846288/ai-bot2-removebg-preview_f7fxim.png" alt="profile-icon" class="profile-icon" style="cursor: pointer;" onclick="setIsCollapsed(false)" />
...
let chatData = [{ "answer": welcomeMessage || "" }];
let query = "";
